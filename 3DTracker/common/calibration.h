                     ...
                     CvMat** reproj_errs, double* avg_reproj_err )
{
    ...
    for( i = 0; i < image_count; i++ )
    {
        cv::Point2f* src_img_pt = (cv::Point2f*)reader.ptr;
        cv::Point2f* dst_img_pt = ((cv::Point2f*)image_points->data.fl) + i*point_count;
...
int doCalibration( int argc, char** argv, cv::Mat &destinationCameraMatrix, cv::Mat &destinationDistortionCoefficients)
{
    CvSize board_size = {0,0};
    float square_size = 1.f, aspect_ratio = 1.f;
    const char* out_filename = "out_camera_data.yml";
    const char* input_filename = 0;
    int i, image_count = 25;
    int write_extrinsics = 0, write_points = 0;
    int flags = 0;
    //CvCapture* capture = 0;
    int capture = 1;
    cv::VideoCapture newCapture;
    FILE* f = 0;
    char imagename[1024];
    ...
    newCapture = cv::VideoCapture(TrackingCommon::cameraIndex);
    ...
    for(;;)
    {
        ...
        else if( true )
        {
            ...
            newCapture >> capturedFrame;
            IplImage tempImage = capturedFrame;
            view0 = &tempImage;
...
calibrate:
            ...
            /** ADDED BLOCK TO RETURN MATRIX TO MAIN 3DTRACKER PROGRAM **/

            cv::Mat resultCameraMatrix = cv::Mat(&camera, true);
            cv::Mat resultDistortionCoefficients = cv::Mat(&dist_coeffs, true);
            resultCameraMatrix.copyTo(destinationCameraMatrix);
            resultDistortionCoefficients.copyTo(destinationDistortionCoefficients);
