...
void MeanShiftTracker::setTargetObject(cv::Mat &Img, cv::Rect trackingWindow)
{
    ...
    // Kalman filter
    cv::Mat currentPose = (cv::Mat_<float>(2, 1) << trackingWindowCenter.x, trackingWindowCenter.y);
    filter.setState(currentPose);
}

void MeanShiftTracker::highlightObject(cv::Mat &outputImage)
{
    if (TrackingCommon::showMeanShiftModel)
    {
    }
...
void MeanShiftTracker::getQu(cv::Mat &Img)
{
	int x, y;
...
void MeanShiftTracker::trackObject(cv::Mat &image) {
	double temp = 0;
    ...
    if (TrackingCommon::enableMeanShiftModelUpdate)
    ...
    if (TrackingCommon::enableFilteringMeanShift) {
        cv::Mat currentPose = (cv::Mat_<float>(2, 1) << trackingWindowCenter.x, trackingWindowCenter.y);
        if (TrackingCommon::enableFilteringMeanShiftToggled) {
            filter.setState(currentPose);
            TrackingCommon::enableFilteringMeanShiftToggled = false;
        }
        filter.filter(currentPose);
        trackingWindowCenter.x = filter.FPose.at<float>(0, 0);
        trackingWindowCenter.y = filter.FPose.at<float>(1, 0);
        //trackingWindowCenterFiltered.x = filter.FPose.at<float>(0, 0);
        //trackingWindowCenterFiltered.y = filter.FPose.at<float>(1, 0);
    }
...
void MeanShiftTracker::getPu(cv::Mat &Img, double pu[colorBins][colorBins][colorBins], cv::Point testPoint)
{
	int x, y;
	double norm = 0, tempC = 0, epani = 0;
	int indR, indG, indB;
...
cv::Point MeanShiftTracker::MeanShift(cv::Mat &Img, double pu[colorBins][colorBins][colorBins], cv::Point testPoint) {
...
void MeanShiftTracker::updateModel(cv::Mat &image, cv::Point p_trackingWindowCenter)
{
