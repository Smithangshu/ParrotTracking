    ...
    while( navdata_option_ptr != NULL )
        {
            ...
            if( navdata_option_ptr->size == 0 )
                ...
                {
                qDebug() << "navdata_option_ptr->tag  " << navdata_option_ptr->tag ;
...
static void navdata_write (int8_t *buffer, int32_t len)
{
	...
	/// Open udp socket to broadcast commands to other mykonos
		...
		if( navdata->header == NAVDATA_HEADER)
		{
			...
			if( navdata->sequence > sequence) 
			{
				...
				if ( get_mask_from_state( mykonos_state, MYKONOS_NAVDATA_DEMO_MASK ))
				{
                    ...
                    //qDebug() << "cks == navdata_cks " << (cks == navdata_cks) << ", " << cks << ", " << navdata_cks;
                    //qDebug() << "msg " << &msg[0] << "size " << (size - sizeof(navdata_cks_t));
                    if( true || cks == navdata_cks )
					{
                    ...
                    //qDebug() << helidata.phi << helidata.psi << helidata.theta << helidata.altitude << helidata.vx << helidata.vy << helidata.vz << helidata.battery;
...
void updateHeliData(double &phi, double &psi, double &theta, double &altitude, double &battery, double &vx, double &vy, double &vz)
{
    phi = helidata.phi;
    psi = helidata.psi;
    theta = helidata.theta;
    altitude = helidata.altitude;
    battery = helidata.battery;
    vx = helidata.vx;
    vy = helidata.vy;
    vz = helidata.vz;
    //qDebug() << "helidata: " << phi << psi << theta << altitude << vx << vy << vz << battery;
}
